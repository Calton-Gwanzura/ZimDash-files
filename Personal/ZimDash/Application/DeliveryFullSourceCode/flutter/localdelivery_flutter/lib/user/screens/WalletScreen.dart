...
class WalletScreenState extends State<WalletScreen> {
  ...
  getWalletData() async {
    ...
    await getWalletList(page: currentPage).then((value) {
      ...
      isAllowToAdd = value.walletBalance!.flag ?? 1;
                ...
                child: Row(
                  ...
                  children: [
                    ...
                    commonButton(language.addMoney, () {
                      if (isAllowToAdd == 1) {
                        toast(language.addWalletBalanceDialogValidation);
                      } else {
                        showDialog(
                          context: context,
                          builder: (context) {
                              ...
                              child: Column(
                                ...
                                children: [
                                  ...
                                  AppTextField(
                                    ...
                                    inputFormatters: [
                                      FilteringTextInputFormatter.allow(RegExp(r'^\d*\.?\d{0,2}$')), // Allows digits and one decimal point with up to 2 decimal places
                                    ],
          ...
          Container(
            child: Column(
              children: [
                ...
                6.height,
                Container(decoration: boxDecorationDefault(border: Border.all(color: Colors.grey.withOpacity(0.2)), color: Colors.transparent), child: Text(language.copy, style: boldTextStyle(color: ColorUtils.colorPrimary, size: 14)).paddingAll(6).center()).onTap(() {
