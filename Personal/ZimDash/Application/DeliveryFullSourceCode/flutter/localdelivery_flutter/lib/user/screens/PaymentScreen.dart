...
class PaymentScreenState extends State<PaymentScreen> {
  ...
  void payPalPayment() async {
    final packageInfo = await getAppInformation();

    final BraintreePayment braintreePayment = BraintreePayment();

    final PayPalAccountNonce? result = await braintreePayment.paypalPayment(
      PayPalRequest(
        currencyCode: appStore.currencyCode,
        token: payPalTokenizationKey!,
        amount: widget.totalAmount.toString(),
        displayName: "Payment for delivery order #${widget.orderId}",
        billingAgreementDescription:
            "Payment for delivery order #${widget.orderId}",
    ...
    if (result?.nonce != null) {
      var request = <String, String?>{
        "txn_id": result!.nonce,
        "description": "Payment for delivery order #${widget.orderId}",
        "paypal_payer_id": result.payerId,
      };
      if (widget.isWallet == true) {
        paymentConfirm(
            paymentType: PAYMENT_TYPE_PAYPAL, transactionId: result.nonce);
      } else {
        savePaymentApiCall(
    ...
    // result => 5b4100d6-8124-18d5-1a0e-566096059ab3
    ...
    //   } else {
    //     savePaymentApiCall(paymentType: PAYMENT_TYPE_PAYPAL, paymentStatus: PAYMENT_PAID, txnId: result.paymentMethodNonce.nonce, transactionDetail: request);
    //   }
    // }
  }
